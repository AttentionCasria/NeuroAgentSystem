...
#
# # 1. 定义工具 (Tools)
# # 这里直接复用你 MedicalAssistant 里的检索逻辑
# retriever = UnifiedSearchEngine(
#     persist_dir=CONFIG["persist_dir"],
#     top_k=CONFIG["top_k_per_store"]
# )
...
#             model="qwen-max",
#             dashscope_api_key=os.getenv("QWEN-API-KEY"),
#             temperature=0  # Agent 需要低随机性
#         )
#         self.tools = [search_medical_guidelines]
#         self.executor = self._create_agent_executor()
#
#     def _create_agent_executor(self):
#         # 注意：这里我们完全不改动你原来的提示词，而是把它们组合成 Agent 的 System Message
#         # 我们使用 LangChain 官方的 Function Calling 结构
#         prompt = ChatPromptTemplate.from_messages([
#             ("system",
#              "You are a professional medical assistant. Use the provided tools to help answer medical questions."),
